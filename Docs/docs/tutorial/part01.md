...
### 比較演算子
比較に関する演算子には以下のものがあります。
比較演算子は`true`または`false`を返します。
比較演算子は整数にも文字列にも使うことができます。
...
### 論理演算子
真偽値すなわち`true`または`false`に関する演算子には以下のものがあります。
...
`(&&)`と`(||)`はショートサーキット評価されます。
すなわち、(&&)の右辺の式が`false`の場合は左辺は計算されず、
(||)の右辺の式が`true`の場合は左辺は計算されません。

### コメント
プログラム中にはコメントを含めることができます。
