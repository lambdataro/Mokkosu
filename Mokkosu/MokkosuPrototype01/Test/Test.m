#============================================================
#! @file	MokkosuQuiz.m
#! @brief	Mokkosu tutorial
#!
#!   このチュートリアルは、香川考司先生（香川大学）の講義資料の問題を
#!   Mokkosuで書き直したものです。
#!
...
do map print_list <| powerset [1,2,3];
...
let queens n =
  let ans = ref [] in
  let check q qs = foldr (&&) true <| mapi {(i,x) -> x<>q && x-i<>q && x+i<>q} qs in
  fun loop qs =
    if (length qs) == n -> ans := qs::!ans
    else for q <- 0 .. (n-1);
             if check q qs;
         in do loop (q::qs) |> ignore in []
  in do loop [] in !ans;

do print_list_with print_list <| queens 8;
#]
